...
- **Firefox needs a forced GL context on a GPU-less runner**: the Linux CI runner has no GPU and Firefox headless blocklists WebGL, so Cesium never renders and `camera.flyTo` never animates — the `firefox` project in `playwright.config.ts` therefore sets `webgl.force-enabled`, `webgl.forbid-software: false`, `gfx.webrender.software` and `layers.acceleration.disabled`. Chromium (SwiftShader) and WebKit render without any of that, so a globe-rendering assertion can fail on Firefox alone while both others pass. When a globe assertion fails only on Firefox, check the GL context before suspecting the command transport.
