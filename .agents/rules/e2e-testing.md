...
- **Headless Firefox cannot do WebGL, so the firefox project runs headed on CI**: Firefox's headless mode has no WebGL support at all (upstream bugzilla 1375585, still NEW — preferences like `webgl.force-enabled` do not conjure a context, and the runner reports `webgl2:false webgl1:false`). Cesium then never renders, `camera.flyTo` never animates, and a globe assertion fails on Firefox alone while Chromium (SwiftShader) and WebKit (their own software renderers) pass. The `firefox` job in `playwright.yml` installs `libgl1-mesa-dri` and runs the project headed under `xvfb-run` with `LIBGL_ALWAYS_SOFTWARE=1`; `playwright.config.ts` sets `headless: false` for that project when `CI` is set. When a globe assertion fails on Firefox alone, check the GL context before suspecting the command transport.
