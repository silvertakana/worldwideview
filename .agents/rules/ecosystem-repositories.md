...
| **`wwv-data-engine-public`** | The V2 Data Engine (Host Environment) that runs the seeders. **Edit here** if you need to modify how the engine parses WebSocket streams, implements REST APIs, downloads seeder bundles, or handles Redis caching. |
...
2. **Fixing Real-Time Streaming Issues:**
   ...
   - Investigate `C:\dev\wwv-data-engine-public` (for backend Fastify websocket broadcasting).
3. **Pnpm Workspaces:**
